...
int main (int argc, char* argv[]) {
  ...
  if (argc < 6) {
		...
		else
		{
			...
			for ( ; j < size && arr[0] < n; j++)
			{
				...
				while (k < 3 && arr[0] < n)
				{
					...
					if (arr[1] > n)
						arr[1] = n;
					MPI_Isend(&arr, 2, MPI_INT, j, 100, MPI_COMM_WORLD, &request);
					arr[0] = arr[1];
					k++;
					totalChunks++;
				}
			...
			{
				arr[0] = n;
				MPI_Isend(&arr, 2, MPI_INT, i, 100, MPI_COMM_WORLD, &request); 
			}
			
			for(int k=0; totalChunks > 0;)
			{
				totalChunks--;
				...
				else
				{
					...
					if (arr[1] > n)
						arr[1] = n;
					MPI_Isend(&arr, 2, MPI_INT, status.MPI_SOURCE, 100, MPI_COMM_WORLD, &request);
					arr[0] = arr[1];
				}
	...
	else
	{
		while(1)
		{
			MPI_Irecv(&arr, 2, MPI_INT, 0, 100, MPI_COMM_WORLD, &request);
			...
			MPI_Isend(&localSum, 1, MPI_FLOAT, 0, 102, MPI_COMM_WORLD, &request);
		}
	}	
	MPI_Finalize();
